...
class Dataloader():
    def __init__(self, maxneigh, batchsize, ncyc=5, initpot=0.0, cutoff=5.0, datafolder="./", ene_shift=True, force_table=True, stress_table=False, dipole_table=False, bec_table=False, cross_val=True, jnp_dtype="float32", key=0, eval_mode=False, Fshuffle=False, ntrain=10):
        ...
        self.nspec = self.reduce_spec.shape[0]
    ...
    def __next__(self):
        ...
        if uppoint < self.numpoint + 0.5:
            ...
            species = self.species[index_batch].reshape(self.ncyc, -1, self.maxnumatom)
            center_factor = self.center_factor[index_batch].reshape(self.ncyc, -1, self.maxnumatom)
            cell = self.cell[index_batch].reshape(self.ncyc, -1, 3, 3).astype(self.float_dtype)
            shiftimage = shiftimage.transpose((0, 2, 1)).reshape(self.ncyc, -1, self.maxneigh, 3).astype(self.float_dtype)
            coor = coor.reshape(self.ncyc, -1, self.maxnumatom, 3).astype(self.float_dtype)
            neighlist = neighlist.reshape(self.ncyc, -1, 2, self.maxneigh).astype(self.int_dtype)
            pot = self.pot[index_batch].reshape(self.ncyc, -1)
            field = self.field[index_batch].reshape(self.ncyc, -1, 3)
            abprop = (pot,)
            if self.force_table:
                force = self.force[index_batch].reshape(self.ncyc, -1, self.maxnumatom, 3)
                abprop = abprop + (force,)
            ...
            if self.bec_table:
                bec = self.bec[index_batch].reshape(self.ncyc, -1, self.maxnumatom, 3, 3)
                abprop = abprop + (bec,)
