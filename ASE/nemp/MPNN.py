...
class MPNN(nn.Module):
    ...
    def setup(self):
        ...
        self.radialcoeff = self.param('radialcoeff', nn.initializers.uniform(12.0), (self.config.nspec, self.config.nspec, self.config.nradial), dtype)
        
        self.init_ead = self.param('init_ead', nn.initializers.normal(1.0), (self.config.nspec, self.config.nspec, self.config.nwave), dtype)

        self.rweight = self.param('rweight', nn.initializers.normal(1.0), (self.config.nspec, self.config.nspec, self.config.nwave), dtype)
        ...
        self.FieldNN_init = MLP.MLP(num_output = self.config.prmaxl*self.config.nwave, num_blocks = self.config.out_nl[0], features = self.config.out_nl[1], layers_per_block = self.config.out_nl[2], use_linear = self.config.out_nl[3], use_bias=False, bias_init_value = None, cst=self.config.cst, dtype=dtype)

        self.FieldNN_list=[MLP.MLP(num_output = self.config.rmaxl*self.config.nwave, num_blocks = self.config.out_nl[0], features = self.config.out_nl[1], layers_per_block = self.config.out_nl[2], use_linear = self.config.out_nl[3], use_bias=False, bias_init_value = None, cst=self.config.cst, dtype=dtype) for iMP_loop in range(self.config.MP_loop)]
    ...
    def __call__(self, cart, field, cell, disp_cell, neighlist, shiftimage, center_factor, species):
        dtype = self.config.initbias_neigh.dtype
        ...
        field_intensity = jnp.sqrt(jnp.sum(jnp.square(field)) + eps)
        sph_field = self.sph_cal(field.reshape(-1, 1) / field_intensity)
        ...
        expand_indices = spec_indices[neighlist]

        radialcoeff = self.radialcoeff + jnp.array(0.1, dtype=dtype)
        emb_coeff = radialcoeff[expand_indices[0], expand_indices[1]]
        rweight = self.rweight[expand_indices[0], expand_indices[1]]
        init_ead = self.init_ead[expand_indices[0], expand_indices[1]]
        smooth_weight = rweight * cut_func[:, None]
        smooth_initead = init_ead * cut_func[:, None]
        radial_func = jnp.sinc(norm_dist[:, None] * emb_coeff) * cut_func[:, None]
        radial_func = jnp.concatenate((smooth_weight, radial_func), axis=1)


        wradial = self.radialnn(radial_func).reshape(-1, 4*prmaxl_i+2, nwave_i)
        ead = jnp.concatenate((smooth_initead, wradial[:, -1]), axis=1)
        density = jnp.zeros((numatom, nwave_i), dtype=dtype).at[neighlist[0]].add(wradial[:, -2])
        density = density / ave_neigh

        pindex_l = self.config.index_l[:pnorb_i]
        field_coeff = self.FieldNN_init(density * field_intensity).reshape(numatom, prmaxl_i, nwave_i) / (field_intensity * jnp.sqrt(dtype_2))
        field_orbital = jnp.einsum("jm, ijk -> ijk", sph_field[:pnorb_i], field_coeff[:, pindex_l])

        worbital = jnp.einsum("ijk, ji -> ijk", wradial[:, pindex_l], sph[:pnorb_i])
        center_orbital = jnp.zeros((numatom, pnorb_i, nwave_i)).at[neighlist[0]].add(worbital)
        center_orbital = center_orbital / (jnp.sqrt(dtype_2) * ave_neigh[:, None]) + field_orbital
        center_orbital = jnp.einsum("ikm, ijk ->ijm", (self.spec_coeff / jnp.sqrt(nwave_f * dtype_2))[spec_indices], center_orbital)
        ...
        for iter_loop in range(self.config.MP_loop):
            ...
            density1 = density1 / ave_neigh
            density = jnp.concatenate((density, density1), axis=1)

            field_coeff = self.FieldNN_list[iter_loop](density * field_intensity).reshape(numatom, rmaxl_i, nwave_i) / (jnp.sqrt(dtype_2) * field_intensity)
            field_orbital = jnp.einsum("jm, ijk -> ijk", sph_field, field_coeff[:, self.config.index_l])
        ...
        atomic_ene = self.outnn(density).reshape(-1)
        atomic_ene = atomic_ene*scale[:, 0] + scale[:, 1]

        return jnp.sum(atomic_ene * center_factor) * jnp.array(self.config.std, dtype=dtype)


    def sum_interaction(self, prmaxl_i, nwave_i, field_orbital, center_orbital, contract_coeff, l_coeff, orb_coeff, neighlist, ave_neigh, pindex_l, sph, dtype_2):
        ...
        init_orb = jnp.zeros_like(field_orbital).at[neighlist[0]].add(worbital)
        init_orb = init_orb / (jnp.sqrt(dtype_2) * ave_neigh[:, None]) + field_orbital
        ...
        norm = ave_neigh * jnp.sqrt(self.config.count_l[pindex_l])
