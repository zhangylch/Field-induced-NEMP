...
Field = jnp.array(np.array(Field).astype(float_dtype))
...
def energy_fn(positions, cell, neighlist):
    ...
    return model.apply(params, positions, Field, cell, disp_cell, jax.lax.stop_gradient(cut_neighlist), jax.lax.stop_gradient(cut_shifts), center_factor, species)
