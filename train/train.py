...
def train(params, ema_params, config, optim, ckpt, ckpt_cpu, ckpt_restore, ckpt_state, ckpt_state_cpu, schedule_fn, value_and_grad_fn, value_fn, data_load, warm_lr, slr, elr, warm_epoch, Epoch, ncyc, ntrain, nval, nprop):
    ...
    def train_loop(nstep):
        ...
        def optimize_epoch(params, opt_state, ema_params, scale, loss_out, weight, data):
            ...
            def body(i, carry):
                params, opt_state, ema_params, scale, weight, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, loss_fn = carry
                inabprop = (iabprop[i] for iabprop in abprop)
                loss, grads = value_and_grad_fn(params, coor[i], field[i], cell[i], disp_cell[i], neighlist[i], shiftimage[i], center_factor[i], species[i], inabprop, weight)
                ...
                loss_fn += loss
                return params, opt_state, ema_params, scale, weight, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, loss_fn
            ...
            tmp_state = opt_state
            tmp_ema = ema_params 
            params, opt_state, ema_params, scale, weight, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, loss_out = \
            jax.lax.fori_loop(0, nstep, body, (params, opt_state, ema_params, scale, weight, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, loss_out))
    ...
    def val_loop(nstep):
        def get_loss(params, scale, loss_out, ploss_out, weight, data):
            def body(i, carry):
                params, weight, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, loss_fn, ploss_fn = carry
                inabprop = (iabprop[i] for iabprop in abprop)
                loss, ploss = value_fn(params, coor[i], field[i], cell[i], disp_cell[i], neighlist[i], shiftimage[i], center_factor[i], species[i], inabprop, weight)
                loss_fn = loss_fn + loss
                ploss_fn = ploss_fn + ploss
                return params, weight, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, loss_fn, ploss_fn

            coor, field, cell, neighlist, shiftimage, center_factor, species, abprop = data
            disp_cell = jnp.zeros_like(cell)
            params, weight, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, loss_out, ploss_out = \
            jax.lax.fori_loop(0, nstep, body, (params, weight, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, loss_out, ploss_out))
...
coor, field, cell, neighlist, shiftimage, center_factor, species, abprop = data_on_gpu0_pytree

initdata = (coor[0, 0], field[0, 0], cell[0, 0], jnp.zeros((3,3)), neighlist[0, 0], shiftimage[0, 0], center_factor[0, 0], species[0, 0])

#=================================================Equi MPNN===================================================================
config = ModelConfig(nspec=nspec, num_cg=num_cg, MP_nl=full_config.MP_nl, radial_nl=full_config.radial_nl, out_nl=full_config.out_nl, reduce_spec=reduce_spec, count_l=count_l, index_l=index_l, index_i1=index_i1, index_i2=index_i2, ens_cg=ens_cg, index_add=index_add, index_den=index_den, index_squ=index_squ, initbias_neigh=initbias_neigh, cutoff=full_config.cutoff, nradial=full_config.nradial, nwave=full_config.nwave, rmaxl=rmaxl, prmaxl=prmaxl, MP_loop=full_config.MP_loop, pn=full_config.pn, npaircode=full_config.npaircode, use_norm=full_config.use_norm, use_bias=full_config.use_bias, std=force_std, cst=1.67462)
...
def make_gradient(pes_model):
    ...
    def wf_loss(params, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, weight):
...
def make_loss(pes_model, nprop):
    ...
    def get_loss(params, coor, field, cell, disp_cell, neighlist, shiftimage, center_factor, species, abprop, weight):
